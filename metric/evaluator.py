...
class TranslationEvaluator:
    ...
    def evaluate(self, data, dims=None, overall=True, print_result=False):
        """
            ...
            dims: A list of dimensions to be evaluated. If dims is None, TranslationEvaluator will evaluate
                  four dimensions: coherence, consistency, fluency, relevance.
...
def get_evaluator(task, model_name_or_path='MingZhong/unieval-sum', max_length=1024, device='cuda:0', cache_dir=None):
    assert task in ['summarization', 'dialogue', 'data2text', 'fact', 'translation', 'qa']
    if task == 'summarization':
        ...
        return TranslationEvaluator(model_name_or_path=model_name_or_path,
                                    max_length=max_length,
                                    device=device,
                                    cache_dir=cache_dir)
    elif task == 'qa':
        return QAEvaluator(model_name_or_path=model_name_or_path,
                                    max_length=max_length,
                                    device=device,
                                    cache_dir=cache_dir)
    else:
